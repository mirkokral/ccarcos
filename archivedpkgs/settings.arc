...
local function reserialize(value)
    ...
    return tutils.u(tutils.s(value))
end
...
local function set_value(name, new)
    ...
    if old ~= new then
        ...
        arcos.queue("setting_changed", name, new, old)
    end
end
...
function load(path)
    ...
    local file = files.open(path or ".settings", "r")
    if not file then
        return false
    end

    local sText = file.read()
    file.close()

    local tFile = tutils.u(sText)
...
function save(path)
    expect(1, path, "string", "nil")
    local file = files.open(path or ".settings", "w")
    if not file then
        return false
    end

    file.write(tutils.s(values))
    file.close()
