...
Gets the width and height of this element.
...
```lua
fun(whence?: string, offset?: string):nil
```
...
Gets all contents of file

## readBytes


```lua
fun(amount: any):number|number[]
```
...
```lua
fun(towrite: string):nil
```

Erases file and writes towrite to it
...
The bg color for the render command
...
The foreground color for the render command
...
The text for the render command
...
The X position for the render command
...
The Y position for the render command
...
The background color of the scroll pane
...
Gets the total height of all the elements
...
The width of the pane. Currently cuts off elements but maybe vertical scroll next time
...
```lua
"back"|"bottom"|"front"|"left"|"right"...(+1)
```
...
```lua
function _G.arcos.createUser(user: string, password: string)
  -> boolean
```
...
```lua
function _G.arcos.deleteUser(user: any)
  -> boolean
```
...
```lua
function _G.arcos.validateUser(user: string, password: string)
  -> boolean
```
...
```lua
function file.flush(towrite: any)
```
...
```lua
function file.readLine(withTrailing: any)
  -> unknown
```
...
```lua
function file.seekBytes(whence: any, offset: any)
```
...
```lua
function file.write(towrite: any)
```
...
```lua
function file.writeLine(towrite: any)
```
...
```lua
function mysplit(inputstr: any, sep: any)
  -> table
```
...
```lua
function _G.read(r?: any, v?: any, a?: any)
  -> userInput: string
```
...
```lua
function _G.strsplit(inputstr: any, sep: any)
  -> table
```
