...
__nexui_Terminal.new = function(printFunction) 
  local self = _hx_new(__nexui_Terminal.prototype)
  __nexui_Terminal.super(self,printFunction)
  return self
end
__nexui_Terminal.super = function(self,printFunction) 
  self.cursorPos = _hx_tab_array({[0]=0, 0}, 2);
  self.printFunction = _hx_funcToField(printFunction);
end
__nexui_Terminal.__name__ = true
__nexui_Terminal.prototype = _hx_e();
__nexui_Terminal.prototype.printFunction = function(self,toprint) 
  _G.io.write(Std.string(toprint));
  _G.io.flush();
end
__nexui_Terminal.prototype.write = function(self,text) 
  self:printFunction(text);
end
__nexui_Terminal.prototype.scroll = function(self,y) 
  self:printFunction("Scrolling not implemented!");
end
__nexui_Terminal.prototype.redirect = function(self,target) 
  if (_hx_bind(target,target.printFunction) ~= nil) then 
    self.printFunction = _hx_funcToField(_hx_bind(target,target.printFunction));
  else
    self.redirect = _hx_funcToField(_hx_bind(target,target.redirect));
    self.current = _hx_funcToField(_hx_bind(target,target.current));
  end;
end
__nexui_Terminal.prototype.native = function(self) 
  do return self end
end
__nexui_Terminal.prototype.current = function(self) 
  do return self end
end
if _hx_bit_raw then
